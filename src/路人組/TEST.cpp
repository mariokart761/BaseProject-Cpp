...
void SearchALLMember()
{
    ...
    while (fscanf(fptr2, "%s\t%s\t%s\t%s", TempStudentName, TempPhoneNum, TempEmail, TempCustomContent) != EOF)
    {
        strcpy(classmate[SearchTeam][i].StudentName, TempStudentName);
        strcpy(classmate[SearchTeam][i].PhoneNum, TempPhoneNum);
        strcpy(classmate[SearchTeam][i].Email, TempEmail);
        strcpy(classmate[SearchTeam][i].CustomContent, TempCustomContent);
        i++;
    }
ChooseMember:
    int selection;
    ...
    for (int j = 0; j < i; j++)
    {
        printf("%d:%s\n", j + 1, classmate[SearchTeam][j].StudentName);
    }
    scanf("%d", &selection);
    if (selection > i)
    {
    ...
    printf("\n姓名 : %s\n電話 : %s\n信箱 : %s\n其他資訊 : %s\n",
           classmate[SearchTeam][selection - 1].StudentName,
           classmate[SearchTeam][selection - 1].PhoneNum,
           classmate[SearchTeam][selection - 1].Email,
           classmate[SearchTeam][selection - 1].CustomContent);
}
...
void GroupingFunction()
{
    ...
    case 4:
        if (AuthorizeLevel == 4)
        {
            SearchALLMember(); //本科教授可查詢所有組別成員
        }
        else if (AuthorizeLevel == 3 || AuthorizeLevel == 1)
        {
            printf("提示:權限不足無法使用\n");
        }
        else
...
void SearchTeammateRating()
{
    ...
    ChooseMember:
        ...
        if (classmate[TeamCheck][selection - 1].grade.count != 0)
        {
            ...
            printf("\n領導能力 : ");
            for (int k = classmate[TeamCheck][selection - 1].grade.LeadershipP; k > 0; k = k - 5)
                printf("▇");
            printf("\n參與程度 : ");
            for (int k = classmate[TeamCheck][selection - 1].grade.ParticipationP; k > 0; k = k - 5)
                printf("▇");
            printf("\n專業技術 : ");
            for (int k = classmate[TeamCheck][selection - 1].grade.SkillP; k > 0; k = k - 5)
                printf("▇");
            printf("\n文書處理 : ");
            for (int k = classmate[TeamCheck][selection - 1].grade.DocumentP; k > 0; k = k - 5)
                printf("▇");
            printf("\n演講能力 : ");
            for (int k = classmate[TeamCheck][selection - 1].grade.SpeechP; k > 0; k = k - 5)
                printf("▇");
            int AvgGrade =
                (classmate[TeamCheck][selection - 1].grade.LeadershipP +
                 ...
                 classmate[TeamCheck][selection - 1].grade.SpeechP) /
                5;
            printf("\n綜合評價 : ");
            for (int k = AvgGrade; k > 0; k = k - 5)
                printf("▇");
            printf("\n");
        }
        else
...
ChooseMember:
    ...
    if (classmate[SearchTeam][selection - 1].grade.count != 0)
    {
        printf("\n%s的評分 : ", classmate[TeamCheck][selection - 1].StudentName);
        printf("\n領導能力 : ");
        for (int k = classmate[SearchTeam][selection - 1].grade.LeadershipP; k > 0; k = k - 5)
            printf("▇");
        printf("\n參與程度 : ");
        for (int k = classmate[SearchTeam][selection - 1].grade.ParticipationP; k > 0; k = k - 5)
            printf("▇");
        printf("\n專業技術 : ");
        for (int k = classmate[SearchTeam][selection - 1].grade.SkillP; k > 0; k = k - 5)
            printf("▇");
        printf("\n文書處理 : ");
        for (int k = classmate[SearchTeam][selection - 1].grade.DocumentP; k > 0; k = k - 5)
            printf("▇");
        printf("\n演講能力 : ");
        for (int k = classmate[SearchTeam][selection - 1].grade.SpeechP; k > 0; k = k - 5)
            printf("▇");
        int AvgGrade =
            (classmate[SearchTeam][selection - 1].grade.LeadershipP +
             classmate[SearchTeam][selection - 1].grade.ParticipationP +
             classmate[SearchTeam][selection - 1].grade.SkillP +
             classmate[SearchTeam][selection - 1].grade.DocumentP +
             classmate[SearchTeam][selection - 1].grade.SpeechP) /
            5;
        printf("\n綜合評價 : ");
        for (int k = AvgGrade; k > 0; k = k - 5)
            printf("▇");
...
void RatingSystem()
{
    ...
    case 1:
        ...
        break;
    case 2:
        if (AuthorizeLevel == 4 || AuthorizeLevel == 1)
        {
            SearchALLRating(); //查詢所有成員評分
            RatingSystem();
        }
        else if (AuthorizeLevel == 3)
        {
            printf("提示:非本科教授無法查詢評分\n");
        }
        else
        {
